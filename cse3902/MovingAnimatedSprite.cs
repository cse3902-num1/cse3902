...
public class MovingAnimatedSprite : ISprite
{
    ...
    public MovingAnimatedSprite(Vector2 position)
    {
        _position = position;
    }
    ...
    public void Update(Game game, GameTime gameTime, List<InputState> inputStates)
    {
        ...
        if (InputState.IsAnyJustPressed(inputStates, InputAction.ShowMovingAnimatedSprite))       _isVisible = true;
        
        /* move left and right */
        double t = gameTime.TotalGameTime.TotalSeconds;
        float ww = game.Window.ClientBounds.Width;
        float w = ww - (_srcRects[_frame].Height * _scale.X);
        _position.X = (float) System.Math.Sin(t) * w/2 + ww/2;
    }
